...
?>
<!doctype html><html lang="ar" dir="rtl"><meta charset="utf-8"><title>Dashboard</title>
<style>
body{font-family:Arial;background:#f4f6f8;margin:0}
.nav{background:#172033;color:#fff;padding:15px}
...
.last-check{white-space:nowrap;font-size:12px;direction:ltr}
.action-cell{white-space:nowrap;min-width:125px}
...
<option value="0">إيقاف</option><option value="10">10 ثواني</option><option value="30">30 ثانية</option><option value="60">1 دقيقة</option><option value="120">2 دقيقة</option><option value="300">5 دقائق</option><option value="600">10 دقائق</option>
</select>
...
<table>
<thead><tr>
<th style="width:4%">#</th><th style="width:11%">السيرفر</th><th style="width:8%">الحالة</th><th style="width:6%">المواقع</th><th style="width:6%">Warning</th><th style="width:6%">Limit</th><th style="width:6%">Load</th><th style="width:6%">RAM</th><th style="width:6%">Disk</th><th style="width:7%">Backup</th><th style="width:16%">Services</th><th style="width:10%">آخر فحص</th><th style="width:12%">إجراء</th>
</tr></thead>
<tbody>
<?php foreach($servers as $s):
$m=latest_metric($pdo,$s['id']);
$sites=(int)($m['accounts'] ?? 0);
$limitReached=($s['website_hard_limit']>0 && $sites >= $s['website_hard_limit']);
$sc=$limitReached?'offline':(($s['website_warning_limit']>0 && $sites >= $s['website_warning_limit'])?'warn':'');
$rowClass=$limitReached?'limit-reached':'';
$diskPercent=isset($m['disk_percent'])?(float)$m['disk_percent']:null;
...
<td class="server-name"><?=htmlspecialchars($s['name'])?></td>
<td class="<?=$s['last_status']==='online'?'online':($s['last_status']==='offline'?'offline':'warn')?>"><?=$s['last_status']==='online'?'🟢 Online':($s['last_status']==='offline'?'🔴 Offline':'🟠 '.$s['last_status'])?></td>
<td class="<?=$sc?>"><?=number_format($sites)?></td>
<td><?=number_format($s['website_warning_limit'])?></td>
<td><?=number_format($s['website_hard_limit'])?></td>
<td><?=htmlspecialchars($m['load_1m']??'-')?></td>
<td><?=isset($m['ram_percent'])?$m['ram_percent'].'%':'-'?></td>
<td class="<?=$diskPercent !== null && $diskPercent >= 90 ? 'disk-warning' : ''?>"><?= $diskPercent !== null ? $diskPercent.'%' : '-' ?></td>
<td class="<?=$backupPercent !== null && $backupPercent >= $backupThreshold ? 'disk-warning' : ''?>"><?= $backupPercent !== null ? $backupPercent.'%' : '-' ?></td>
<td class="services-cell">Apache <?=in_array(($m['apache_status']??''),['active','enabled'],true)?'🟢':'🔴'?> MySQL <?=in_array(($m['mysql_status']??''),['active','enabled'],true)?'🟢':'🔴'?> DNS <?=in_array(($m['dns_status']??''),['active','enabled'],true)?'🟢':'🔴'?> Exim <?=in_array(($m['exim_status']??''),['active','enabled'],true)?'🟢':'🔴'?></td>
<td class="last-check"><?=htmlspecialchars($s['last_checked_at']??'-')?></td>
...
let secondsLeft=0;let timer=null;
const saved=localStorage.getItem('monitor_refresh_interval');
select.value=saved!==null?saved:'60';
function updateCountdown(){
 if(secondsLeft>=60){const min=Math.floor(secondsLeft/60),sec=secondsLeft%60;countdown.textContent='التحديث التالي: '+min+':'+String(sec).padStart(2,'0');}
 else countdown.textContent='التحديث التالي: '+secondsLeft+' ثانية';
}
function startRefresh(){clearInterval(timer);secondsLeft=parseInt(select.value||'0',10);if(secondsLeft<=0){countdown.textContent='متوقف';return;}updateCountdown();timer=setInterval(function(){secondsLeft--;if(secondsLeft<=0){location.reload();return;}updateCountdown();},1000);}
select.addEventListener('change',function(){localStorage.setItem('monitor_refresh_interval',select.value);startRefresh();});
startRefresh();
})();
</script>
